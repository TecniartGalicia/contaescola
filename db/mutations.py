...
def save_partida(d: dict) -> int:
    """Guarda en la tabla global 'partidas' (sin curso_id)."""
    if d.get("id"):
        mut("UPDATE partidas SET nome=?,saldo_inicial=?,notas=?,activa=? WHERE id=?",
            (d["nome"], d.get("saldo_inicial",0), d.get("notas",""),
             1 if d.get("activa",True) else 0, d["id"]))
        return d["id"]
    return mut(
        "INSERT OR REPLACE INTO partidas (nome,saldo_inicial,notas,activa) VALUES (?,?,?,1)",
        (d["nome"], d.get("saldo_inicial",0), d.get("notas","")),
    )
...
# ── Partidas config legacy (mantener para compatibilidad) ────────
def save_partida_config(d: dict) -> int:
    if d.get("id"):
        mut("UPDATE partidas_config SET nome=?,importe_asignado=?,notas=? WHERE id=?",
