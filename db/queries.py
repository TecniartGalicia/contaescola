...
# ── Partidas globales (nuevo modelo) ────────────────────────────
def get_partidas(solo_activas: bool = False) -> list[dict]:
    """Devuelve todas las partidas globales."""
    sql = "SELECT * FROM partidas"
    if solo_activas: sql += " WHERE activa=1"
    return q(sql + " ORDER BY nome")

def get_partida(id: int) -> dict | None:
    return q1("SELECT * FROM partidas WHERE id=?", (id,))

def get_movs_partida(
    nome: str,
    curso_id: int | None = None,
    ano: int | None = None,
) -> list[dict]:
    """
    Movementos do diario asociados a unha partida global.
    Filtros opcionales por curso escolar ou ano natural.
    """
    sql = """
        SELECT d.*,
...
def get_partidas_resumen_global(nome: str) -> dict:
    """Totales de una partida sin filtro — toda su vida."""
    rows = q(
        "SELECT tipo, SUM(importe) as t FROM diario WHERE xustifica=? GROUP BY tipo",
        (nome,),
    )
...
def get_diario(area: str, ano: int, curso_id: int | None = None) -> list[dict]:
    sql = """SELECT d.*, c.nome AS curso_nome, cl.nome AS cliente_nome,
                    cl.nif AS cliente_nif, cl.direccion AS cliente_dir
             FROM diario d
             LEFT JOIN cursos   c  ON d.curso_id   = c.id
             LEFT JOIN clientes cl ON d.cliente_id  = cl.id
...
def get_diario_partida(xustifica: str, curso_id: int) -> list[dict]:
    return q("""SELECT d.*, c.nome AS curso_nome, cl.nome AS cliente_nome
               FROM diario d
               LEFT JOIN cursos   c  ON d.curso_id  = c.id
               LEFT JOIN clientes cl ON d.cliente_id = cl.id
...
def get_informes(params: dict) -> list[dict]:
    sql = """SELECT d.*, c.nome AS curso_nome, cl.nome AS cliente_nome,
             ...
             FROM diario d
             LEFT JOIN cursos   c  ON d.curso_id   = c.id
             LEFT JOIN clientes cl ON d.cliente_id  = cl.id WHERE 1=1"""
    p: list = []
    ...
    for pr in provs:
        ...
        for per in PERIODOS:
            rows = q("""SELECT tipo, SUM(importe) as t FROM diario
                       WHERE ano=? AND cliente_id=? AND periodo=? GROUP BY tipo""",
                     (ano, pr["id"], per))
            trim[per] = {
