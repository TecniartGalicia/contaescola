...
def get_movs_partida(nome: str, curso_id: int | None = None,
...
def get_partidas_resumen_global(nome: str) -> dict:
    rows = q("SELECT tipo, SUM(importe) as t FROM diario WHERE xustifica=? GROUP BY tipo",
             (nome,))
...
def get_partida_saldos(partida_id: int) -> list[dict]:
